...
void Response::send(const std::string& message) {
    ...
    std::vector<char> cpyVecMsg(responseMsg.begin(), responseMsg.end());
    char* responseMsg_c = cpyVecMsg.data();

    this->clientSocket->writeSocket(responseMsg_c, cpyVecMsg.size());
